...
- **Starter characters** — ships with two example personas (Yamuna and
  Yallamma) pre-seeded on first run, so it's not a blank slate; edit or
  delete them like any other character (`server/src/seedPersonas.ts`)
